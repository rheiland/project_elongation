...
sc=ones(1,10)*11;% select what to cut, see below
% sc=11;
...
for i=1:size(cutinput,1)
    [T,Y,T1,Y1,pressure]=gastrulation(N,boundary,boundaryc,NewN,chemoN,tipN,NCN,ncp,alpha,beta,gamma,theta,Tcut,Tcut2,cutinput(i,:));
    Data{i,1}=cutinput(i,:);
    Data{i,2}=T;
    Data{i,3}=Y;
    Data{i,4}=T1;
